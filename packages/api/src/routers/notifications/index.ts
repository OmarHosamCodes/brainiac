...
export const notificationsRouter = {
  ...
  unreadCount: protectedProProcedure
    ...
    .handler(async ({ context, input }) => {
      return z
        ...
        .parse(await getUnreadNotificationCount(context.session.user.id, input.teamId));
    }),
  markSeen: protectedProProcedure
    ...
    .handler(async ({ context, input }) => {
      return z
        ...
        .parse(await markNotificationsSeen(context.session.user.id, input.teamId));
    }),
  ...
  markAllRead: protectedProProcedure
    ...
    .handler(async ({ context, input }) => {
      return z
        .object({ updated: z.boolean() })
        .parse(await markAllNotificationsRead(context.session.user.id, input.teamId));
    }),
  preferences: {
    get: protectedProProcedure
      .input(teamScopedNotificationInputSchema)
      .handler(async ({ context, input }) => {
        const result = await getNotificationPreferences(context.session.user.id, input.teamId);
