...
- Styling should stay on the standard shadcn theme tokens (light and dark); prefer base theme tokens over custom ones; use shadcn primitives (e.g. Checkbox) for shared filter controls instead of custom/native inputs; do not add custom styling to shadcn UI components unless needed; when applying an external theme pack, keep the product fonts.
