...
- Primary app navigation is a left sidebar rail shared by Dashboard and Agency: team controls at the top (not a logo), notification card at the bottom, reference-style profile card, and the sidebar collapse button lives in the top bar; the sidebar and top bar must read as one connected shell surface.
- Styling should stay on the standard shadcn theme tokens (especially in light theme); do not add custom styling to shadcn UI components unless needed.
