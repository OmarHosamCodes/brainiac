...
async function main() {
  ...
  await run("bun", ["run", "src/seed.ts", "--scale", "massive"]);

  console.log("Seeding massive agency data...");
  await run("bun", ["run", "src/seed-agency.ts", "--yes", "--scale", "massive"]);

  console.log("Granting lifetime pro to founder...");
  await run("bun", ["run", "src/grant-lifetime-pro.ts", "--", "--email", "founder@brainiac.test"]);
