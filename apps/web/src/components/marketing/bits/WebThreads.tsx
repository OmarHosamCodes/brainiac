...
const ctxMap = new WeakMap<HTMLElement, ThreadsContext>();
...
export default function WebThreads({
  ...
  useEffect(() => {
    ...
    // ponytail: window listeners so the effect tracks the cursor when used as a
    // pointer-events-none hero background. Upgrade: pass a target element prop.
    ...
    const loop = (t: number) => {
      ...
      mouse[0] = currentMouse[0] ?? 0.5;
      mouse[1] = currentMouse[1] ?? 0.5;
