...
const canAnimate = () => {
  if (supportsRoll === undefined) {
    supportsRoll =
      ...
      CSS.supports("transform", "translateY(clamp(-1lh, calc((mod(7.5, 10) - 5) * 1lh), 1lh))");
    if (supportsRoll) {
      try {
...
function NumberRollDigit({ digit, dir }: { digit: number; dir: number }) {
  ...
  return (
    ...
    >
      ...
      <span data-d={digit} className="invisible before:content-[attr(data-d)]" />
...
function NumberRollPart({ part, dir }: { part: RenderedPart; dir: number }) {
  return (
    <span
      ...
      style={
        {
          "--aui-number-roll-shift": dir === 0 ? "0%" : dir > 0 ? "35%" : "-35%",
        } as CSSProperties
      }
    >
...
export type NumberRollProps = Omit<ComponentProps<"span">, "children" | "prefix"> & {
  value: number;
...
function NumberRoll({
  ...
  if (display.formatted !== formatted) {
    setDisplay({
      ...
      dir: trend === "up" ? 1 : trend === "down" ? -1 : Math.sign(value - display.value),
    });
  }
    ...
    for (const key of exiting) {
      ...
      timers.set(
        ...
        setTimeout(() => {
          ...
          setDisplay((current) => ({
            ...current,
            rendered: current.rendered.filter((part) => !(part.exiting && part.key === key)),
          }));
        }, duration),
      );
    }
      ...
      style={
        {
          ...
          "--aui-number-roll-fade": "calc(var(--aui-number-roll-duration) * 0.6)",
