  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        {children ?? (
          <Button variant="outline" size="sm" className="aui-mcp-config-trigger gap-2">
      ...
      <DialogContent className="aui-mcp-config-content sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Connect to Model Context Protocol servers to expose their tools to this assistant.
          </DialogDescription>
        </DialogHeader>
...
const SectionTitle: FC<{ children: ReactNode }> = ({ children }) => (
  <h3 className="text-muted-foreground text-xs font-medium tracking-wide uppercase">{children}</h3>
);
...
const STATUS_VARIANT: Record<MCPConnectionState, "default" | "secondary" | "destructive"> = {
  connected: "default",
  ...
  return (
    <div className="text-muted-foreground flex items-center gap-1.5 text-xs">
      <Badge variant={variant}>
        {status === "connecting" && <Loader2Icon className="size-3 animate-spin" />}
        {label}
      </Badge>
    </div>
  );
...
const ServerActions: FC = () => (
  <div className="flex flex-wrap gap-2">
    <McpServerPrimitive.ConnectButton asChild>
      <Button size="sm" variant="default" className="aui-mcp-server-connect h-8 gap-2 text-xs">
    ...
    <McpServerPrimitive.DisconnectButton asChild>
      <Button size="sm" variant="outline" className="aui-mcp-server-disconnect h-8 text-xs">
        Disconnect
      </Button>
...
const FormRow: FC<{ label: string; children: ReactNode }> = ({ label, children }) => (
