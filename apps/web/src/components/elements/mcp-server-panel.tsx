...
export type McpServerStatus = "connected" | "connecting" | "needs-auth" | "failed";
        ...
        return (
          <div key={server.id} className="flex flex-col">
            ...
            >
              ...
              <span className="min-w-0 flex-1 truncate text-[13.5px]">{server.name}</span>
              <span className={cn(mono, "text-foreground/30 shrink-0")}>
                {server.tools.length} tools
              </span>
              <span
                aria-hidden
                className={cn("size-1.5 shrink-0 rounded-full", DOT[server.status])}
              />
            ...
            {expanded && (
              <div className="fade-in slide-in-from-top-1 animate-in flex flex-col gap-1.5 px-1.5 ps-8 pb-2 duration-200">
                <div className="flex items-center gap-2">
                  <span className={cn(mono, "text-foreground/30")}>{server.transport}</span>
                  <span className={cn(mono, "text-foreground/30")}>· {LABEL[server.status]}</span>
                ...
                <div className="flex flex-wrap gap-1">
                  {server.tools.map((tool) => (
                    <span
                      key={tool}
                      className={cn(field, mono, "text-foreground/55 rounded-md px-1.5 py-0.5")}
                    >
                      {tool}
                    </span>
                  ))}
