...
> & {
  ...
  const matches = threads.filter((thread) =>
    `${thread.title} ${thread.preview}`.toLowerCase().includes(query.toLowerCase()),
  );
  ...
  const row = (thread: SearchableThread) => (
    <button
      ...
      className={cn(
        ...
        thread.id === activeId ? "bg-foreground/[0.05]" : "hover:bg-foreground/[0.03]",
      )}
    >
      <span className="flex items-center gap-1.5">
        {thread.pinned && <PinIcon className="text-foreground/30 size-2.5 shrink-0" />}
        <span className="min-w-0 flex-1 truncate text-[13px]">{thread.title}</span>
      </span>
      <span className="text-foreground/35 truncate text-xs">{thread.preview}</span>
    </button>
  );
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-1.5 rounded-2xl p-3", className)}
      {...props}
    >
      ...
      {pinned.length > 0 && (
        <div className="flex flex-col">
          <span className={cn(mono, "text-foreground/25 px-2 pb-1")}>pinned</span>
          {pinned.map(row)}
        </div>
      )}
      ...
      {groups.map((group) => (
        <div key={group} className="flex flex-col">
          <span className={cn(mono, "text-foreground/25 px-2 pb-1")}>{group}</span>
          {matches.filter((thread) => !thread.pinned && thread.group === group).map(row)}
        </div>
      ))}
