  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          <span className="flex items-baseline gap-2">
            ...
            <span className={cn(mono, "text-foreground/30 shrink-0")}>v{version}</span>
          </span>
          <span className="text-foreground/45 truncate text-xs">{provider}</span>
        </div>
      </div>

      <p className="text-foreground/60 text-xs leading-relaxed">{description}</p>

      <div className="flex flex-col gap-1.5">
        {skills.map((skill) => (
          <div key={skill.name} className="flex items-baseline gap-2">
            <span
              className={cn(field, mono, "text-foreground/55 shrink-0 rounded-md px-1.5 py-0.5")}
            >
              {skill.name}
            </span>
      ...
      <div className="border-foreground/[0.07] flex items-center gap-2 border-t pt-3">
        <span className={cn(mono, "text-foreground/30 min-w-0 flex-1 truncate")}>{endpoint}</span>
      ...
      <button
        ...
        disabled={connected}
        className={cn(
          connected ? cn(field, "text-foreground/55") : cn(inkButton, "justify-center"),
