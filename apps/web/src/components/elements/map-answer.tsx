...
export function MapAnswer({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "pins" | "activeId" | "route" | "onSelect"> & {
        ...
        {pins.map((pin) => (
          <button
            ...
            className={cn(
              ...
              pin.id === activeId ? "bg-foreground/[0.03]" : "hover:bg-foreground/[0.02]",
            )}
          >
            <span className="text-foreground/85 min-w-0 flex-1 truncate text-[13px]">
              {pin.label}
            </span>
            <span className={cn(mono, "text-foreground/30 shrink-0")}>{pin.detail}</span>
          </button>
        ))}
      </div>
    </div>
