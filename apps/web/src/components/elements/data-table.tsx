...
export function DataTable({ rows, cycle, className, ...props }: DataTableProps) {
  return (
    <div
    ...
    >
      <div className="flex items-center px-4 pt-3 pb-2">
        ...
        <span className={cn(mono, "text-foreground/35 w-16 text-end")}>Context</span>
        <span className={cn(mono, "text-foreground/35 w-16 text-end")}>Cost</span>
      </div>
      ...
      <div key={cycle}>
        {rows.map((row, index) => (
          ...
          >
            ...
            <span className="text-foreground/90 flex-1 truncate">{row.name}</span>
            <span className={cn(mono, "text-foreground/55 w-16 text-end tabular-nums")}>
              {row.context}
            </span>
            <span className={cn(mono, "text-foreground/55 w-16 text-end tabular-nums")}>
              {row.cost}
