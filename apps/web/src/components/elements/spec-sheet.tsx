...
export function SpecSheet({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "title" | "subtitle" | "rows" | "visibleCount"> & {
  title: string;
  subtitle?: string;
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-0.5">
        ...
        {subtitle && <span className="text-foreground/45 text-xs">{subtitle}</span>}
      </div>
      ...
      <div className="flex flex-col">
        {take(rows, visibleCount).map((row) => (
          ...
          >
            <span className={cn(mono, "text-foreground/35 w-24 shrink-0")}>{row.label}</span>
            <span
              className={cn(
                "min-w-0 flex-1 text-end text-[13px] tabular-nums",
                row.emphasis ? "text-foreground/95 font-medium" : "text-foreground/70",
              )}
            >
              {row.value}
            </span>
