...
export function TraceWaterfall({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "spans" | "totalMs" | "visibleCount"> & {
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-md flex-col gap-2 rounded-2xl p-4", className)}
      {...props}
    >
      <div className="flex items-baseline justify-between">
        <span className="text-[13.5px] font-medium">Trace</span>
        <span className={cn(mono, "text-foreground/35 tabular-nums")}>{totalMs}ms</span>
      </div>
        ...
        {take(spans, visibleCount).map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={cn(mono, "text-foreground/30 text-end tabular-nums")}>
                {item.durationMs}
              </span>
            </div>
          );
