...
export function PromptLibrary({
  ...
  ComponentProps<"div">,
  "children" | "prompts" | "query" | "selectedId" | "onQueryChange" | "onSelect" | "onInsert"
> & {
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-2 rounded-2xl p-3", className)}
      {...props}
    >
      ...
      <div id={listId} role="listbox" aria-label="Saved prompts" className="flex flex-col">
        {matches.map((prompt) => (
          <button
            key={prompt.id}
            ...
            className={cn(
              ...
              prompt.id === selectedId ? "bg-foreground/[0.05]" : "hover:bg-foreground/[0.03]",
            )}
          >
