...
export function MathBlock({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "label" | "steps" | "visibleSteps"> & {
  label?: string;
  ...
  return (
    <div
      ...
      className={cn(paper, "flex w-full max-w-sm flex-col gap-2.5 rounded-2xl p-4", className)}
      {...props}
    >
      ...
      {take(steps, visibleSteps).map((step, i) => (
        ...
        >
          ...
          {step.note && (
            <span className={cn(mono, "text-foreground/30 text-center")}>{step.note}</span>
          )}
        </div>
      ))}
    </div>
...
export function Frac({ over, under }: { over: React.ReactNode; under: React.ReactNode }) {
  return (
    <span
      data-slot="frac"
