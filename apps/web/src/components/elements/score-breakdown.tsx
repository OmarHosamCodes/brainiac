  ...
  return (
    ...
    >
      <div className="flex items-baseline gap-2">
        <span className="text-2xl font-medium tracking-tight tabular-nums">{total.toFixed(1)}</span>
        <span className={cn(mono, "text-foreground/30 tabular-nums")}>/ {outOf}</span>
        <span
          className={cn(
        ...
        {take(criteria, visibleCount).map((criterion) => (
          ...
          >
            <div className="flex items-baseline gap-2">
              ...
              <span className={cn(mono, "text-foreground/25 shrink-0")}>×{criterion.weight}</span>
