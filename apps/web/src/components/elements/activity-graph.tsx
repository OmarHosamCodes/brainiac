...
export function ActivityGraph({
  ...
  className,
  ...props
}: Omit<ComponentProps<"div">, "children" | "data" | "start" | "end" | "title" | "total"> & {
  ...
  return (
    ...
    >
      <div className="flex items-baseline justify-between">
        ...
        <span className={cn(mono, "text-foreground/35 tabular-nums")}>{total}</span>
      </div>
        ...
        <div className="flex gap-2 overflow-x-auto">
          <div className="flex shrink-0 flex-col gap-[3px]">
            <HeatGraph.DayLabels>
              {({ label }) => (
                <span
                  className={cn(mono, "text-foreground/25 flex h-[9px] items-center leading-none")}
                >
                  {label.row % 2 === 1 ? HeatGraph.DAY_SHORT[label.dayOfWeek] : ""}
                </span>
              )}
            </HeatGraph.DayLabels>
          </div>
          ...
          <HeatGraph.Grid className="gap-[3px]">
            {({ cell }) => (
              <HeatGraph.Cell
                className={cn("size-[9px] rounded-[2px]", LEVEL_TINT[cell.level] ?? LEVEL_TINT[0])}
              />
            )}
          </HeatGraph.Grid>
        </div>
