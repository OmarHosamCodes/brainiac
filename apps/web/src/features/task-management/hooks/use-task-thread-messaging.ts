...
import { useAgencyTaskMessagesInfiniteQuery } from "@/features/shared/agency-queries";
...
import type { AgencyTaskMessage } from "@/features/task-management/agency-work";

import { useAgencyVoiceRecorder } from "@/features/task-management/hooks/use-agency-voice-recorder";
import { useTaskThreadLiveSync } from "@/features/task-management/hooks/use-task-thread-live-sync";
import { useTaskThreadScroll } from "@/features/task-management/hooks/use-task-thread-scroll";
...
export function useTaskThreadMessaging({
  ...
  const scrollAnchorKey =
    (messages.length > 0 ? messages[messages.length - 1]?.animationKey : null) ?? "empty";
