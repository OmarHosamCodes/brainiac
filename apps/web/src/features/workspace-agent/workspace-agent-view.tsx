...
import { getMessageText } from "@/features/workspace-agent/orch-ui-message";
import { WorkspaceAgentElementsChromeView } from "@/features/workspace-agent/workspace-agent-elements-chrome-view";
...
export function WorkspaceAgentView({ view }: WorkspaceAgentViewProps) {
  ...
  const lastAssistantWords = getMessageText(
    [...view.messages].reverse().find((message) => message.role === "assistant") ?? {
      id: "empty",
      role: "assistant",
      parts: [],
    },
  ...
  const lastUserText = getMessageText(
    [...view.messages].reverse().find((message) => message.role === "user") ?? {
      id: "empty",
      role: "user",
      parts: [],
    },
            ...
            <AnimatePresence initial={false} mode="popLayout">
              {!view.expanded ? (
                ...
                >
                  ...
                  {showChat ? (
                    <WorkspaceAgentElementsChromeView
                      isStreaming={view.isStreaming}
                      streamStopped={view.streamStopped}
                      error={view.error}
                      onRetry={() => {
                        if (lastUserText.trim()) void view.sendMessage({ text: lastUserText });
                      }}
                      onContinue={() => {
                        if (lastUserText.trim()) void view.sendMessage({ text: lastUserText });
                      }}
                      lastAssistantWords={lastAssistantWords}
                      quota={view.quotaBanner}
                    />
                  ) : null}
