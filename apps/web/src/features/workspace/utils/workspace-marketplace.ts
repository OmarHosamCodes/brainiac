...
export function createNodeMarketplacePayload(node: WorkspaceNode): WorkspaceMarketplacePayload {
  return workspaceMarketplacePayloadSchema.parse({ kind: "node", node: cloneForTransport(node) });
}
...
export function cloneMarketplaceTabPayload(payload: WorkspaceMarketplacePayload) {
  ...
  return { templates, tab: cloneWorkspaceTabForInsertion(payload.tab, templateIdMap, timestamp) };
}
...
export function getMarketplacePayloadTypeLabel(payload: WorkspaceMarketplacePayload) {
  if (payload.kind === "node") return "Node";
  if (payload.kind === "tab") return "Tab";
  return "Block";
}
export function getMarketplacePayloadSummary(payload: WorkspaceMarketplacePayload) {
  if (payload.kind === "node") return `${payload.node.tabs.length} tabs`;
  if (payload.kind === "tab") return `${payload.tab.blocks.length} blocks`;
