...
export function AgencyProjectsVirtualTable({
  ...
  return (
    <div className="overflow-x-auto rounded-2xl border border-default bg-default">
      <div
        className={cn(
          "grid min-w-[46rem] grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_5.5rem] border-b border-default bg-muted text-xs",
          agencyLabelClass,
        )}
      >
      ...
      <div
        ref={listRef}
        className="relative min-w-[46rem]"
        ...
        {virtualizer.getVirtualItems().map((virtualRow) => {
          ...
          return (
            <div
              key={project.id}
              className={cn(
                "absolute top-0 left-0 grid w-full grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_5.5rem] border-b border-default text-xs transition-colors hover:bg-elevated/40",
