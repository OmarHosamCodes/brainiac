...
const blockAgentPromptMutation = useMutation(
    orpc.agent.chat.turn.mutationOptions(),
);
...
const activeTab = computed(() => {
    ...
    return (
        node.value.tabs.find((tab) => tab.id === activeTabId.value) ??
        node.value.tabs[0] ??
        null
    );
});
...
const normalizedBlockSearch = computed(() =>
    blockSearch.value.trim().toLowerCase(),
);
...
function mutateCurrentNode(
    mutator: (entry: WorkspaceNode, timestamp: string) => void,
) {
...
function mutateTypedBlock<TBlockType extends WorkspaceBlock["type"]>(
    ...
    mutateBlock(tabId, blockId, (block, tab, nodeEntry, timestamp) => {
        ...
        mutator(
            block as Extract<WorkspaceBlock, { type: TBlockType }>,
            tab,
            nodeEntry,
            timestamp,
        );
...
function openTabEditor(mode: WorkspaceTabEditorMode) {
    ...
    tabEditor.mode = mode;
    tabEditor.title =
        mode === "rename" ? getDisplayTabTitle(activeTab.value) : "";
    tabEditor.open = true;
}
...
function deleteActiveTab() {
    ...
    const currentIndex = node.value.tabs.findIndex(
    ...
    mutateCurrentNode((entry) => {
        ...
        const nextTab =
            entry.tabs[currentIndex] ??
            entry.tabs[Math.max(0, currentIndex - 1)] ??
            entry.tabs[0];
...
function addBlockToActiveTab(type: WorkspaceBlock["type"]) {
    ...
    if (
        ...
        if (
            ...
            preferredAgencyTeamId =
                teams.value[selectedIndex]?.id ?? preferredAgencyTeamId;
        }
    }
    ...
    if (isAgencyOperationsBlock && !preferredAgencyTeamId) {
        toast.add({
            title: "Team required",
            description:
                "Create or join a team before adding Agency Operations blocks.",
            color: "warning",
        });
        return;
    }
...
function mutateCollectedTask(
    ...
    updateDraftNodes((nodes) => {
        ...
        if (block?.type === "content-pipeline") {
            const pipelineItem = block.items.find(
        ...
        if (
            !block ||
            (block.type !== "task-list" && block.type !== "eisenhower-matrix")
        ) {
            return;
        }
...
function mutateTrackerEntry(
    ...
    mutateBlock(tabId, blockId, (block) => {
        ...
        const trackerEntry = block.entries.find(
...
function removeKanbanColumn(tabId: string, blockId: string, columnId: string) {
    mutateBlock(tabId, blockId, (block) => {
        ...
        const nextColumns = block.columns.filter(
...
function mutateTimelineMilestone(
    ...
    milestoneId: string,
    mutator: (
        milestone: ReturnType<typeof createWorkspaceTimelineMilestone>,
    ) => void,
) {
    mutateBlock(tabId, blockId, (block) => {
        if (block.type !== "timeline") {
...
function removeTimelineMilestone(
    tabId: string,
    blockId: string,
    milestoneId: string,
) {
...
function mutateScorecardMetric(
    ...
    metricId: string,
    mutator: (
        metric: ReturnType<typeof createWorkspaceScorecardMetric>,
    ) => void,
) {
    mutateBlock(tabId, blockId, (block) => {
        if (block.type !== "scorecard") {
...
function removeScorecardMetric(
    tabId: string,
    blockId: string,
    metricId: string,
) {
    mutateBlock(tabId, blockId, (block) => {
        ...
        block.metrics = block.metrics.filter(
...
async function runPromptBlock(tabId: string, blockId: string) {
    ...
    const block = activeTab.value?.blocks.find(
        (
            candidate,
        ): candidate is Extract<WorkspaceBlock, { type: "ai-prompt" }> =>
    ...
    const scopedNodes = block.includeContext
        ? cloneWorkspaceNodes([node.value])
        : undefined;
...
async function runBlockAgentPrompt(
    tabId: string,
    blockId: string,
    prompt: string,
) {
...
function getTimeOrchestratorSummaryForBlock(
...
function createAgentContextNode(
    ...
    clonedNode.title =
        `${getDisplayBlockTitle(block)} - ${nodeEntry.title}`.slice(0, 120);
    ...
    scopedTab.blocks = scopedTab.blocks.filter(
...
function getBlockSearchText(block: WorkspaceBlock) {
    ...
    if (block.type === "task-list") {
        ...
        fragments.push(
            ...block.pros.flatMap((item) => [
                item.text,
                String(item.weight),
                "pro",
            ]),
            ...block.cons.flatMap((item) => [
                item.text,
                String(item.weight),
                "con",
            ]),
        ...
        fragments.push(
            ...block.members.flatMap((member) => [
                member.name,
                member.role,
                ...Object.entries(member.scores).flatMap(
                    ([dimension, score]) => [dimension, String(score)],
                ),
            ]),
        );
        ...
        fragments.push(
            ...
            ...block.expenses.flatMap((expense) => [
                expense.category,
                String(expense.amountEgp),
            ]),
        );
    ...
    if (block.type === "task-list") {
        ...
        details.push(
            ...block.members.flatMap((member) => [
                member.name,
                member.role,
                ...Object.entries(member.scores).map(
        ...
        details.push(
            ...
            ...block.options.flatMap((option) => [
                option.label,
                ...Object.values(option.scores).map(
                    (score) => `Score ${score}`,
                ),
            ]),
        );
...
function getBlockSearchMatches(block: WorkspaceBlock) {
    ...
    return collectBlockSearchDetails(block)
        .filter((detail) =>
            detail.toLowerCase().includes(normalizedBlockSearch.value),
        )
        .slice(0, 3);
}
...
function highlightSearchMatch(value: string) {
    ...
    const pattern = new RegExp(
        escapeForRegex(normalizedBlockSearch.value),
        "ig",
    );
...
function getCustomTemplate(definitionId: string) {
    return (
        node.value?.customBlockTemplates.find(
...
function getCustomFormulaResult(block: WorkspaceCustomBlock) {
    ...
    return evaluateCustomBlockFormula(
        template?.formula?.expression,
        block.values,
    );
}
...
<template>
    <div class="relative h-full w-full overflow-hidden">
        ...
        >
            <!-- Sidebar Skeleton -->
            <div
                class="w-80 border-r border-muted/30 bg-default/40 p-6 space-y-8"
            >
                <div class="space-y-4">
        ...
        <template v-else-if="node && activeTab">
            <div class="flex h-full min-h-0 w-full overflow-hidden">
                <div class="min-w-0 flex flex-1 flex-col">
                    <div class="min-h-0 flex-1">
                        <WorkspaceNodeShell
                            ...
                            :node-visibility-badge-class="
                                nodeVisibilityBadgeClass
                            "
                            :node-owner-label="nodeOwnerLabel"
                            :node-team-name="
                                canManageNodeSharing
                                    ? (activeTeamMembership?.name ??
                                      node.teamId ??
                                      null)
                                    : null
                            "
                            ...
                            :unshare-pending="
                                unshareNodeMutation.isPending.value
                            "
                            @update:node-share-team-id="
                                nodeShareTeamId = $event
                            "
                ...
                >
                    ...
                    <div
                        class="sticky top-0 flex h-full min-h-0 flex-col gap-3 p-3"
                    >
                        <div class="min-h-0 flex-1">
...
.agent-rail-aura {
    ...
    background:
        radial-gradient(
            34rem 26rem at 12% 22%,
            rgb(59 130 246 / 0.16),
            transparent 62%
        ),
        radial-gradient(
            26rem 20rem at 88% 18%,
            rgb(16 185 129 / 0.12),
            transparent 58%
        ),
        radial-gradient(
            20rem 18rem at 50% 78%,
            rgb(99 102 241 / 0.1),
            transparent 60%
        );
    filter: blur(10px);
