...
const workspaceReadyForEdits = computed(
  () => Boolean(session.value?.data?.user) && loadApplied.value && !isHydratingWorkspace.value,
);
...
async function persistWorkspace(snapshot: WorkspaceNode[]) {
  ...
  try {
    ...
    if (requestId === latestSaveRequest) {
      ...
      saveError.value = error instanceof Error ? error.message : "Failed to save workspace";
    }
  }
}
...
watch(
  ...
  (remoteNodes) => {
    ...
    nodes.value = remoteNodes.map((node) => normalizeNode(node as WorkspaceNode));
  ...
  <div class="h-full min-h-0 p-4 md:p-6">
    <div class="flex h-full min-h-0 flex-col gap-4">
      <UAlert
        ...
        :description="workspaceQuery.error.value?.message || 'The user workspace could not be loaded.'"
      />
      ...
      <div class="min-h-0 flex-1">
        ...
        >
          <template #node="{ node, selected }">
            <UCard
              class="node-card h-full border-0 bg-transparent shadow-none ring-0"
              :ui="{
                body: 'flex h-full min-h-0 flex-col px-5 pb-5 pt-4 sm:p-5',
                header: 'px-5 pt-5 sm:px-5',
              }"
            >
              <template #header>
                <div class="flex items-start justify-between gap-4">
                  <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.3em] text-muted">Workspace Node</p>
                    <h3 class="mt-2 text-lg font-semibold text-highlighted">{{ node.title }}</h3>
                  </div>
                  <div
                    class="rounded-full border px-3 py-1 text-xs font-medium"
                    :class="selected ? 'border-primary/50 bg-primary/10 text-primary' : 'border-muted/60 bg-elevated/80 text-toned'"
                  >
              ...
              <p class="text-sm leading-6 whitespace-pre-wrap text-toned">
                {{ node.content || 'No content yet. Use the context menu to edit this node.' }}
              </p>

              <div class="mt-auto grid gap-3 pt-5 text-sm sm:grid-cols-2">
                <div class="rounded-2xl border border-muted/60 bg-elevated/70 p-4">
                  <div class="text-xs uppercase tracking-[0.2em] text-muted">Author</div>
                  <div class="mt-2 font-semibold text-highlighted">
                    {{ session?.data?.user?.name || 'Workspace owner' }}
                  </div>
                </div>
                <div class="rounded-2xl border border-muted/60 bg-elevated/70 p-4">
                  <div class="text-xs uppercase tracking-[0.2em] text-muted">Updated</div>
                  <div class="mt-2 font-semibold text-highlighted">
      ...
      <div class="pointer-events-none fixed bottom-6 left-1/2 z-20 -translate-x-1/2 px-4">
        <div class="pointer-events-auto flex items-center gap-3 rounded-full border border-muted/70 bg-default/90 px-4 py-2 shadow-lg shadow-black/5 backdrop-blur-md">
          <div>
            <div class="text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-muted">Workspace</div>
            <div class="text-sm font-semibold text-highlighted">
              {{ session?.data?.user?.name ? `${session.data.user.name}'s board` : 'Personal board' }}
            </div>
          </div>
          ...
          <div class="rounded-full border px-3 py-1 text-xs font-medium" :class="saveBadge.className">
            {{ saveBadge.label }}
          </div>
        </div>
      </div>
    ...
    >
      <template #body>
        <UFormField label="Title" name="title" required>
          <UInput v-model="nodeDraft.title" placeholder="Strategy lane" autofocus />
        </UFormField>

        <UFormField label="Content" name="content" description="Keep it concise enough to scan while navigating the board.">
          <UTextarea
      ...
      <template #footer>
        <UButton color="neutral" variant="ghost" @click="closeEditor">Cancel</UButton>
        <UButton :disabled="!isDraftValid" icon="i-lucide-save" @click="submitNodeEditor">
