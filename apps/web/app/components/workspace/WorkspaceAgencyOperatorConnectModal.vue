...
const boundTeams = computed(() => teams.value.filter((team) => props.boundTeamIds.has(team.id)));
...
<template>
  <UModal :open="open" :ui="{ content: 'max-w-lg' }" @update:open="emit('update:open', $event)">
    <template #content>
      <div class="p-6">
        <!-- Header -->
        <div class="mb-6 flex items-center gap-3.5">
          <div
            class="flex size-10 shrink-0 items-center justify-center rounded-2xl bg-emerald-500/12 text-emerald-600 dark:bg-emerald-400/12 dark:text-emerald-400"
          >
        ...
        <div v-if="activePanel === 'select'" class="space-y-4">
          ...
          <template v-else>
            ...
            <div v-if="boundTeams.length > 0" class="space-y-1.5">
              ...
              >
                <div
                  class="flex size-8 shrink-0 items-center justify-center rounded-xl bg-neutral-100 text-[11px] font-bold uppercase text-neutral-500 dark:bg-neutral-800"
                >
                  {{ team.name.slice(0, 2).toUpperCase() }}
                </div>
                <div class="min-w-0 flex-1">
