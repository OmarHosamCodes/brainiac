...
function toggleTag(tag: AgencyTag) {
  ...
  editDraft.value.tags = exists ? current.filter((t) => t.id !== tag.id) : [...current, tag];
}
...
function flagLabel(reason: FlagReason): string {
  switch (reason) {
    case "zero-duration":
      return "Zero/near-zero duration";
    case "missing-description":
      return "Missing description";
    case "long-entry":
      return `Over ${LONG_ENTRY_HOURS}h`;
    case "ai-flag":
      return "Flagged by AI scan";
  }
}
...
<template>
  <div class="space-y-5">
    ...
    <div
      class="flex items-center justify-between gap-3 rounded-2xl border border-zinc-800/50 bg-zinc-950/50 px-4 py-3 backdrop-blur-sm"
    >
      <div class="flex items-center gap-2">
        ...
        <div class="text-center">
          ...
          <p v-if="periodOffset === 0" class="text-xs text-emerald-500">Current period</p>
    ...
    >
      ...
      <div class="grid gap-3 sm:grid-cols-3">
        ...
        <UFormField label="Period start day" size="sm">
          <UInput v-model.number="billingStartDay" type="number" :min="1" :max="28" size="sm" />
        </UFormField>
        <UFormField label="Period end day" size="sm">
          <UInput v-model.number="billingEndDay" type="number" :min="1" :max="28" size="sm" />
        </UFormField>
      </div>
    ...
    <!-- ── Filters ── -->
    <div
      class="grid gap-3 sm:grid-cols-3 rounded-2xl border border-zinc-800/50 bg-zinc-950/40 p-4 backdrop-blur-sm"
    >
    ...
    <div
      v-else
      class="overflow-hidden rounded-2xl border border-zinc-800/50 bg-zinc-950/40 backdrop-blur-sm"
    >
      <table class="w-full text-sm">
        <thead>
          <tr class="border-b border-zinc-800/70">
            <th
              class="px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-widest text-zinc-500"
            >
              Member
            </th>
            <th
            ...
            <th
              class="px-3 py-2.5 text-center text-xs font-semibold uppercase tracking-widest text-zinc-500"
            >
              Status
            </th>
            <th
              class="px-3 py-2.5 text-right text-xs font-semibold uppercase tracking-widest text-zinc-500"
            >
              Actions
            </th>
          </tr>
        ...
        <tbody>
          <template v-for="entry in entries" :key="entry.id">
            ...
            <!-- ── Edit row ── -->
            <tr v-else class="border-b border-emerald-900/40 bg-zinc-900/80">
              <td class="px-3 py-2.5">
                    ...
                    <template #content>
                      <div class="w-72 space-y-2 p-2">
                        ...
                        <div class="flex items-center justify-between gap-2">
                          <p v-if="editDraftLinkState.error" class="text-[11px] text-error">
              ...
              <td class="px-3 py-2.5">
                <input
                  ...
                  @change="
                    editDraft.startAt = datetimeLocalToIso(
              ...
              <td class="px-3 py-2.5">
                <input
                  ...
                  class="rounded-lg border border-zinc-700 bg-zinc-900 px-2 py-1 font-mono text-xs text-zinc-200 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  @change="
                    editDraft.endAt = datetimeLocalToIso(($event.target as HTMLInputElement).value)
                  "
                />
              </td>
              <td class="px-3 py-2.5">
