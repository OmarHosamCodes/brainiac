...
function toggleHabitDay(habitId: string, day: (typeof WORKSPACE_HABIT_GRID_DAYS)[number]) {
  mutateHabit(habitId, (habit) => {
  ...
  <div class="space-y-6">
    ...
    <div class="space-y-4 rounded-3xl border border-muted/20 bg-elevated/10 p-5">
      ...
      <div class="flex flex-wrap items-center gap-2">
        <UBadge variant="subtle" class="rounded-2xl"> {{ summary.totalHabits }} habits </UBadge>
          ...
          <tr v-for="habit in block.habits" :key="habit.id" class="group">
            <td class="px-3">
              <UInput
                ...
                @update:model-value="updateHabitName(habit.id, $event)"
              />
            </td>
            <td
              v-for="day in WORKSPACE_HABIT_GRID_DAYS"
            ...
            >
              ...
              >
                <span class="sr-only">{{ dayLabels[day] }}</span>
                <UIcon
