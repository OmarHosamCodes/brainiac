...
function removeColumn(columnId: string) {
    mutateTypedBlock(props.tabId, props.block.id, "table", (entry) => {
        ...
        entry.columns = entry.columns.filter(
...
function getRowFilledCellCount(row: WorkspaceTableBlock["rows"][number]) {
    return props.block.columns.filter((column) =>
        Boolean(row.cells[column.id]?.trim()),
    ).length;
}
</script>
...
<template>
    <div class="space-y-6">
        <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="grid flex-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <p
                            class="text-2xl sm:text-3xl font-black tracking-tight text-primary"
                        >
            ...
            <div
                class="mt-6 flex flex-wrap items-center gap-2 border-t border-muted/10 pt-6"
            >
                <UButton
                    color="neutral"
                    variant="soft"
        ...
        <div
            class="overflow-x-auto rounded-3xl border border-muted/20 bg-default/40 p-4"
        >
            <table class="min-w-full border-separate border-spacing-y-3">
                        ...
                        >
                            <div class="flex items-center gap-2">
                                <UInput
                                    ...
                                    @update:model-value="
                                        updateColumnLabel(column.id, $event)
                                    "
                                />
                                <UButton
                                    color="neutral"
                ...
                <tbody>
                    <tr v-for="(row, rowIndex) in block.rows" :key="row.id">
                        <th
                            scope="row"
                            class="px-2 align-middle text-sm font-bold text-muted/60"
                        >
                            <div class="space-y-1">
                                <p>Row {{ rowIndex + 1 }}</p>
                                <p
                        ...
                        >
                            <UInput
                                ...
                                @update:model-value="
                                    updateCellValue(row.id, column.id, $event)
                                "
                            />
                        </td>
