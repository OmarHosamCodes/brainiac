...
const dimensionIds = computed(() => props.block.dimensions.map((dimension) => dimension.id));
...
const strongestDimensionAverage = computed(() => {
  ...
  return summary.value.averageByDimension[summary.value.strongestDimension] ?? 0;
});
...
function removeDimension(dimensionId: string) {
  mutateHeatMap((block) => {
    ...
    block.dimensions = block.dimensions.filter((dimension) => dimension.id !== dimensionId);
...
function updateDimensionLabel(dimensionId: string, value: string | number | undefined) {
  mutateHeatMap((block) => {
    const target = block.dimensions.find((dimension) => dimension.id === dimensionId);
...
function updateMemberName(memberId: string, value: string | number | undefined) {
  mutateHeatMap((block) => {
...
function updateMemberRole(memberId: string, value: string | number | undefined) {
  mutateHeatMap((block) => {
    const target = block.members.find((member) => member.id === memberId);
...
function getDimensionLabel(dimensionId: string) {
  return props.block.dimensions.find((dimension) => dimension.id === dimensionId)?.label ?? "Skill";
}
...
function getMemberLabel(member: WorkspaceSkillsHeatMapBlock["members"][number]) {
...
function getRemoveMemberLabel(member: WorkspaceSkillsHeatMapBlock["members"][number]) {
  ...
  <div class="space-y-6">
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
        ...
        <p class="mt-1 text-[10px] font-bold uppercase tracking-widest text-muted/60">
          {{
            ...
            >
              <div class="flex flex-col items-center gap-1 group">
                <UInput
                  ...
                  @update:model-value="updateDimensionLabel(dimension.id, $event)"
                />
                <UButton
          ...
          >
            ...
            >
              <UInput
                ...
                @update:model-value="updateMemberName(member.id, $event)"
              />
              <UInput
            ...
            <td class="border-y border-muted/20 bg-default/40 px-3 py-4 text-center">
              <div
                ...
                :class="getScoreClasses(getMemberAverage(member))"
              >
