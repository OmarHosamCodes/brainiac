...
const summaryByOptionId = computed(
    () =>
        new Map(
            summary.value.optionScores.map((option) => [
                option.optionId,
                option,
            ]),
        ),
...
function updateOptionLabel(
    ...
    mutateTypedBlock(
        ...
        "decision-matrix",
        (entry) => {
            const target = entry.options.find(
...
function updateCriterionWeight(
    criterionId: string,
    value: string | number | undefined,
) {
    mutateTypedBlock(
...
function removeCriterion(criterionId: string) {
    mutateTypedBlock(
        ...
        (entry) => {
            ...
            entry.criteria = entry.criteria.filter(
            ...
            entry.options.push(
                createWorkspaceDecisionMatrixOption({
                    ...
                    scores: Object.fromEntries(
                        entry.criteria.map((criterion) => [criterion.id, 5]),
                    ),
                }),
            );
        },
...
function removeOption(optionId: string) {
    mutateTypedBlock(
        ...
        (entry) => {
            ...
            entry.options = entry.options.filter(
    ...
    <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="space-y-1">
                    ...
                    <p class="text-sm text-muted">
                        Compare options with weighted criteria, then score each
                        path from 0 to 10.
                    </p>
                </div>
            ...
            >
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0 flex-1">
                        ...
                        <UInput
                            ...
                            @update:model-value="
                                updateOptionLabel(option.id, $event)
                            "
                        />
                ...
                <div class="mt-4 space-y-2">
                    ...
                    >
                        ...
                        <span
                            >{{
                                getOptionSummary(option.id)?.progress ?? 0
                            }}%</span
                        >
                    </div>
                    <UProgress
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 px-1">
            <div>
                ...
                <p class="text-sm text-muted">
                    Increase criterion weight when it matters more, then score
                    each option against that criterion.
                </p>
            </div>
        ...
        <div class="overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0">
            ...
            >
                ...
                >
                    <p
                        class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60 truncate"
                    >
                        {{ option.label || "Option" }}
                    </p>
                    ...
                    <p
                        class="text-[10px] font-bold uppercase tracking-widest text-muted/40"
                    >
                        Rank #{{ getOptionRank(option.id) ?? "—" }}
                    </p>
                </div>
                ...
                <template
                    v-for="criterion in block.criteria"
                    :key="criterion.id"
                >
                    <div
                        class="space-y-3 bg-default/40 p-4 flex flex-col justify-center"
                        role="rowheader"
                    >
                        <div class="flex items-start justify-between gap-3">
                            <UInput
                                ...
                                @update:model-value="
                                    updateCriterionLabel(criterion.id, $event)
                                "
                            />
                    ...
                    >
                        ...
                        >
                            <span>Score</span>
                            <span
                                >{{ option.scores[criterion.id] ?? 0 }}/10</span
                            >
                        </div>
                        ...
                        <div
                            class="mt-3 flex justify-between text-xs font-medium text-toned/70"
                        >
                            <span>Weighted</span>
