...
function addItem() {
  mutateTypedBlock(props.tabId, props.block.id, "content-pipeline", (entry) => {
    entry.items.unshift(
      createWorkspaceContentPipelineItem({
        title: "",
        status: "ideas",
...
function moveItemByOffset(itemId: string, offset: -1 | 1) {
  const currentStatus = props.block.items.find((item) => item.id === itemId)?.status;
...
function canMoveItem(itemStatus: WorkspaceContentPipelineStatus, offset: -1 | 1) {
  ...
  return (
    currentIndex + offset >= 0 && currentIndex + offset < WORKSPACE_CONTENT_PIPELINE_STATUSES.length
  );
}
      ...
      >
        <div class="flex items-start justify-between">
          ...
          <div class="size-10 rounded-2xl bg-secondary/10 flex items-center justify-center">
            <UIcon
              :name="
                summary.topPlatform ? getPlatformIcon(summary.topPlatform) : 'i-lucide-trending-up'
              "
            ...
            <div class="flex items-center gap-3">
              ...
              >
                <UIcon
                  :name="column.icon"
                  class="size-4"
                  :class="column.dotClass.replace('bg-', 'text-')"
                />
              </div>
              <div>
                <p
          ...
          <div class="flex-1 space-y-3 p-2 min-h-[400px]">
            ...
            >
              ...
              <div class="mt-4 flex flex-wrap items-center gap-2">
                <USelectMenu
                  :model-value="platformOptions.find((option) => option.value === item.platform)"
