...
const platformOptions = WORKSPACE_CONTENT_PLATFORMS.map((platform) => ({
    ...
    label: string;
    value: WorkspaceContentPlatform;
    icon: string;
}>;
...
const itemsByStatus = computed(
    () =>
        Object.fromEntries(
            ...
            WorkspaceContentPipelineBlock["items"]
        >,
);
...
function moveItemByOffset(itemId: string, offset: -1 | 1) {
    const currentStatus = props.block.items.find(
        (item) => item.id === itemId,
    )?.status;
    ...
    const currentIndex =
        WORKSPACE_CONTENT_PIPELINE_STATUSES.indexOf(currentStatus);
    ...
    const nextStatus =
        WORKSPACE_CONTENT_PIPELINE_STATUSES[currentIndex + offset];
...
function canMoveItem(
    itemStatus: WorkspaceContentPipelineStatus,
    offset: -1 | 1,
) {
    const currentIndex =
        WORKSPACE_CONTENT_PIPELINE_STATUSES.indexOf(itemStatus);
...
<template>
    <div class="space-y-8">
        ...
        <div class="grid gap-4 grid-cols-2 lg:grid-cols-4">
            <div
                class="group relative overflow-hidden rounded-[24px] bg-elevated/5 p-5 border border-muted/10 transition-all hover:bg-elevated/10"
            >
                <div class="flex items-start justify-between">
                    <div>
                        <p
                            class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/50"
                        >
                            Total Pieces
                        </p>
                        <p
                            class="mt-2 text-3xl font-black tracking-tight text-highlighted"
                        >
                            {{ summary.totalItems }}
                        </p>
                    </div>
                ...
                <p class="mt-4 text-xs text-muted/60 leading-relaxed">
            ...
            >
                <div class="flex items-start justify-between">
                    ...
                    <div
                        class="size-10 rounded-2xl bg-secondary/10 flex items-center justify-center"
                    >
                        <UIcon
                            :name="
                                summary.topPlatform
                                    ? getPlatformIcon(summary.topPlatform)
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 px-1">
            <div class="space-y-1">
                ...
                <p class="text-xs text-muted">
                    Drag items between stages or use the move buttons for
                    keyboard-safe updates.
                </p>
            </div>
        ...
        <!-- Pipeline Board -->
        <div
            class="overflow-x-auto pb-6 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-thin scrollbar-thumb-muted/20 snap-x snap-mandatory"
        >
            <div class="flex gap-6">
                ...
                >
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            >
                                <UIcon
                                    ...
                                    class="size-4"
                                    :class="
                                        column.dotClass.replace('bg-', 'text-')
                                    "
                                />
                            </div>
                            <div>
                    ...
                    <div class="flex-1 space-y-3 p-2 min-h-[400px]">
                        ...
                        >
                            ...
                            <div class="mt-4 flex flex-wrap items-center gap-2">
                                <USelectMenu
                                    ...
                                    :ui="{
                                        base: 'rounded-full bg-elevated/5 border-muted/10',
                                    }"
                                    @update:model-value="
                                        mutateItem(item.id, (entry) => {
                                            entry.platform =
                        ...
                        >
                            <div
                                class="size-10 rounded-full bg-muted/5 flex items-center justify-center mb-2"
                            >
                                <UIcon
                                    :name="column.icon"
                                    class="size-5 text-muted/20"
                                />
                            </div>
                            <p
                                class="text-[10px] font-bold uppercase tracking-wider text-muted/30"
                            >
                                No {{ column.status }}
                            </p>
                        </div>
