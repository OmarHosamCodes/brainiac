...
const totalOutcomeCount = computed(() =>
    props.block.courses.reduce(
        (count, course) => count + course.outcomes.length,
        0,
    ),
);
...
function removeCourse(courseId: string) {
    mutateTypedBlock(props.tabId, props.block.id, "course-roadmap", (block) => {
        block.courses = block.courses.filter(
...
function updateLessonTitle(
    ...
    mutateCourse(courseId, (course) => {
        const target = course.lessons.find(
...
function removeOutcome(courseId: string, outcomeId: string) {
    mutateCourse(courseId, (course) => {
        course.outcomes = course.outcomes.filter(
...
function updateOutcomeText(
    ...
    mutateCourse(courseId, (course) => {
        const target = course.outcomes.find(
            ...
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p class="text-sm text-muted">
                        Track lesson recording progress per course and keep the
                        promised outcomes visible.
                    </p>
                </div>
        ...
        <div v-else class="space-y-5">
            ...
            >
                    ...
                    <div class="min-w-0 flex-1 space-y-3">
                        <div class="flex flex-wrap items-center gap-3">
                            <UInput
                                ...
                                @update:model-value="
                                    updateCourseName(course.id, $event)
                                "
                            />
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            >
                                {{
                                    getCourseProgress(course).completionPercent
                                }}% complete
                            </UBadge>
                            <UBadge
                                color="neutral"
                ...
                <div class="mt-5 space-y-3">
                    ...
                    <div
                        class="flex gap-2 overflow-x-auto pb-4 -mx-4 px-4 sm:mx-0 sm:px-0"
                    >
                        <button
