...
const overdueCount = computed(
  () => props.block.invoices.filter((invoice) => getReceivableDaysOverdue(invoice) > 0).length,
);
const highRiskCount = computed(
  () => props.block.invoices.filter((invoice) => getReceivableRiskLevel(invoice) === "high").length,
);
const paidCount = computed(
  () => props.block.invoices.filter((invoice) => invoice.status === "paid").length,
);
...
function mutateInvoice(
  ...
  mutateTypedBlock(props.tabId, props.block.id, "collections-tracker", (entry) => {
...
function updateClientName(invoiceId: string, value: string | number | undefined) {
...
function updateNextFollowUpDate(invoiceId: string, value: string | number | undefined) {
  mutateInvoice(invoiceId, (invoice) => {
    invoice.nextFollowUpDate = String(value ?? "") || null;
  });
}

function updateStatus(invoiceId: string, value: WorkspaceReceivableStatus | string | undefined) {
  mutateInvoice(invoiceId, (invoice) => {
    invoice.status =
  ...
  <div class="space-y-6">
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div class="rounded-3xl border border-primary/20 bg-primary/10 p-5">
        ...
        <p class="mt-2 text-xl font-black tracking-tight text-primary sm:text-2xl">
          {{ formatCurrency(summary.totalOutstanding) }}
        </p>
        <p class="mt-1 text-sm text-muted">{{ props.block.invoices.length }} tracked invoices</p>
      </div>
      ...
      <div class="rounded-3xl border border-error/20 bg-error/10 p-5">
        ...
        <p class="mt-2 text-xl font-black tracking-tight text-error sm:text-2xl">
          {{ formatCurrency(summary.overdueAmount) }}
        </p>
        <p class="mt-1 text-sm text-muted">{{ overdueCount }} invoices need follow-up</p>
      </div>
      ...
      <div class="rounded-3xl border border-warning/20 bg-warning/10 p-5">
        ...
        <p class="mt-2 text-xl font-black tracking-tight text-warning sm:text-2xl">
          {{ formatCurrency(summary.dueThisWeekAmount) }}
        </p>
        <p class="mt-1 text-sm text-muted">{{ highRiskCount }} high-risk exposures</p>
      </div>
      ...
      <div class="rounded-3xl border border-success/20 bg-success/10 p-5">
        ...
        <p class="mt-2 text-xl font-black tracking-tight text-success sm:text-2xl">
          {{ formatCurrency(summary.collectedThisMonth) }}
        </p>
        <p class="mt-1 text-sm text-muted">{{ paidCount }} invoices marked paid</p>
      </div>
    </div>
    ...
    <div class="flex flex-wrap items-start justify-between gap-3 px-1">
      <div class="space-y-2">
        ...
        <div class="flex flex-wrap items-center gap-2">
          ...
          <UBadge v-if="highRiskCount > 0" color="warning" variant="soft" class="rounded-2xl">
        ...
        <template v-for="invoice in filteredInvoices" :key="invoice.id">
          <div class="bg-default/40 p-3">
            <UInput
              ...
              @update:model-value="updateClientName(invoice.id, $event)"
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            <UInput
              ...
              @update:model-value="updateAmount(invoice.id, $event)"
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            <UInput
              ...
              @update:model-value="updateDueDate(invoice.id, $event)"
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            <UInput
              ...
              @update:model-value="updateOwner(invoice.id, $event)"
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            <UInput
              ...
              @update:model-value="updateNextFollowUpDate(invoice.id, $event)"
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            <USelect
              ...
              @update:model-value="
                updateStatus(invoice.id, $event as WorkspaceReceivableStatus | undefined)
              "
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            ...
            <UInput
              ...
              @update:model-value="updatePaidAt(invoice.id, $event)"
            />
          </div>
          ...
          <div class="bg-default/40 p-3">
            <UTextarea
              ...
              @update:model-value="updateNotes(invoice.id, $event)"
            />
          </div>
