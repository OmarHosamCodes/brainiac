...
<template>
  <div class="space-y-8">
    ...
    <div class="rounded-3xl bg-elevated/10 p-6 border border-muted/20 space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div class="text-center">
          ...
          <p class="text-2xl sm:text-3xl font-black tracking-tight text-success">
            {{ summary.prosWeight }}
          </p>
        </div>
        ...
        <div class="flex-1 px-4 sm:px-8">
          ...
          <div class="mt-4 text-center">
            <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60 mb-1">
              Current Signal
            </p>
        ...
        <div class="text-center">
          ...
          <p class="text-2xl sm:text-3xl font-black tracking-tight text-error">
            {{ summary.consWeight }}
          </p>
        </div>
      </div>
          ...
          >
            <div class="flex items-start gap-3">
              <UInput
                ...
                @update:model-value="updateDecisionItemText('pros', item.id, $event)"
              />
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 px-2">
              ...
              <div class="flex flex-wrap items-center gap-2">
                <button
                  ...
                  :class="getWeightButtonClass('pros', item.weight, w)"
                  :aria-label="`Set pro weight to ${w}`"
                  @click="updateDecisionItemWeight('pros', item.id, w)"
                >
                  {{ w }}
                </button>
              </div>
          ...
          >
            <div class="flex items-start gap-3">
              <UInput
                ...
                class="flex-1"
                :ui="{ base: 'px-2 py-1 text-sm font-medium' }"
                @update:model-value="updateDecisionItemText('cons', item.id, $event)"
              />
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 px-2">
              ...
              <div class="flex flex-wrap items-center gap-2">
                <button
                  ...
                  type="button"
                  class="min-w-8 rounded-full border px-2.5 py-1 text-[10px] font-bold transition-colors"
                  :class="getWeightButtonClass('cons', item.weight, w)"
                  :aria-label="`Set con weight to ${w}`"
                  @click="updateDecisionItemWeight('cons', item.id, w)"
                >
                  {{ w }}
                </button>
              </div>
    ...
    <div class="rounded-3xl border border-primary/20 bg-primary/5 p-6 space-y-4">
      <div class="flex items-center gap-2 text-primary">
        ...
        <h3 class="text-[10px] font-bold uppercase tracking-[0.2em] text-primary/80">
          Final Recommendation
        </h3>
      </div>
      <UTextarea
