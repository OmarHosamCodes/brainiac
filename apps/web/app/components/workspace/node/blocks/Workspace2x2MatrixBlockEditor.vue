...
function updateMatrixField(
  field: "xAxisLabel" | "yAxisLabel" | "xStartLabel" | "xEndLabel" | "yStartLabel" | "yEndLabel",
...
function addQuadrantItem(quadrantKey: (typeof quadrants)[number]["key"]) {
  mutateTypedBlock(props.tabId, props.block.id, "2x2-matrix", (entry) => {
    entry.quadrants[quadrantKey].items.push(createWorkspace2x2MatrixItem({ text: "" }));
  });
}
...
function updateQuadrantItem(
  ...
  mutateTypedBlock(props.tabId, props.block.id, "2x2-matrix", (entry) => {
    const target = entry.quadrants[quadrantKey].items.find((candidate) => candidate.id === itemId);
...
function removeQuadrantItem(quadrantKey: (typeof quadrants)[number]["key"], itemId: string) {
  mutateTypedBlock(props.tabId, props.block.id, "2x2-matrix", (entry) => {
    entry.quadrants[quadrantKey].items = entry.quadrants[quadrantKey].items.filter(
    ...
    <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
      <div class="flex flex-wrap items-start justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <UBadge variant="subtle" class="rounded-2xl"> {{ summary.itemCount }} items </UBadge>
    ...
    <div class="grid gap-4 lg:grid-cols-[1fr_auto]">
      ...
      >
        <UInput
          ...
          @update:model-value="updateMatrixField('xAxisLabel', $event, 80)"
        />
        <UInput
          ...
          @update:model-value="updateMatrixField('yAxisLabel', $event, 80)"
        />
        <UInput
          ...
          @update:model-value="updateMatrixField('xStartLabel', $event, 60)"
        />
        <UInput
          ...
          @update:model-value="updateMatrixField('xEndLabel', $event, 60)"
        />
        <UInput
          ...
          @update:model-value="updateMatrixField('yStartLabel', $event, 60)"
        />
        <UInput
          ...
          @update:model-value="updateMatrixField('yEndLabel', $event, 60)"
        />
      </div>
    </div>
          ...
          <div class="mb-3 flex items-center justify-between gap-3">
            <div class="min-w-0 flex-1 space-y-2">
              <div class="flex items-center gap-2">
                <UIcon name="i-lucide-square-dashed" class="size-4" :class="quadrant.accent" />
                <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60">
                  {{ block.quadrants[quadrant.key].items.length }}
                  items
                </p>
              </div>
              <UInput
                ...
                @update:model-value="updateQuadrantName(quadrant.key, $event)"
              />
            </div>
            <UButton
              color="primary"
          ...
          <div class="space-y-2">
            ...
            >
              <UInput
                ...
                @update:model-value="updateQuadrantItem(quadrant.key, item.id, $event)"
              />
              <UButton
                color="neutral"
                variant="ghost"
                ...
                @click="removeQuadrantItem(quadrant.key, item.id)"
              />
            </div>
