...
const stageOptions = (
    ["lead", "consultation", "proposal", "negotiation", "closed"] as const
).map((stage) => ({
...
function updateTemperature(
    dealId: string,
    value: WorkspaceSalesTemperature | string | undefined,
) {
    mutateDeal(dealId, (deal) => {
        deal.temperature =
...
function updateStage(
    dealId: string,
    value: WorkspaceSalesPipelineStage | string | undefined,
) {
    mutateDeal(dealId, (deal) => {
        deal.stage =
            value === "lead" ||
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
                ...
                <p class="mt-1 text-sm text-muted">
                    {{ summary.dealCount }} active deals
                </p>
            </div>
            ...
            <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
                ...
                <p class="mt-1 text-sm text-muted">
                    Prioritization score out of 100
                </p>
            </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 px-1">
            <div>
                ...
                <p class="text-sm text-muted">
                    Deals are automatically ranked by score so the best
                    opportunities stay at the top.
                </p>
            </div>
