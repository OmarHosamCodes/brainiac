...
const trackerSummary = computed(() => ({
    entryCount: props.block.entries.length,
    latestLabel: latestEntry.value?.label || "No entries yet",
    goalLabel:
        props.block.goal === null || props.block.goal === undefined
            ? "No goal"
            : formatStatValue(props.block.goal),
}));
const chartHeights = computed(() => {
    ...
    const min = Math.min(
        ...values,
        props.block.goal ?? Number.POSITIVE_INFINITY,
    );
    const max = Math.max(
        ...values,
        props.block.goal ?? Number.NEGATIVE_INFINITY,
    );
    ...
    return values.map((value) =>
        Math.max(8, Math.round(((value - min) / (max - min)) * 100)),
    );
});
const goalPosition = computed(() => {
    ...
    return Math.max(
        ...
        Math.min(
            100,
            Math.round(((props.block.goal - min) / (max - min)) * 100),
        ),
    );
});
...
function updateGoal(value: string | number | undefined | null) {
    mutateTypedBlock(props.tabId, props.block.id, "tracker", (entry) => {
...
function updateEntryLabel(entryId: string, value: string | number | undefined) {
    mutateTrackerEntry(props.tabId, props.block.id, entryId, (item) => {
        item.label = String(value ?? "").slice(0, 120);
    });
}

function updateEntryValue(entryId: string, value: string | number | undefined) {
    mutateTrackerEntry(props.tabId, props.block.id, entryId, (item) => {
        item.value = toNumber(String(value ?? "0"));
    });
}
</script>
...
<template>
    <div class="space-y-8">
        <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted">
                        Track progress over time, compare against a goal, and
                        keep key entries easy to scan.
                    </p>
                </div>
            ...
            <div class="mt-4 flex flex-wrap items-center gap-2">
                <UBadge variant="subtle" class="rounded-2xl">
                    {{ trackerSummary.entryCount }} entries
                </UBadge>
                <UBadge color="neutral" variant="soft" class="rounded-2xl">
                    {{ trackerSummary.goalLabel }}
                </UBadge>
                <UBadge color="neutral" variant="soft" class="rounded-2xl">
                    {{ trackerSummary.latestLabel }}
                </UBadge>
            </div>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
                ...
                <div class="mt-2 flex items-center gap-2">
                    ...
                    />
                    <p
                        class="text-2xl sm:text-3xl font-black tracking-tight text-highlighted"
                    >
                        {{ trend.delta > 0 ? "+" : ""
            ...
            <div class="rounded-3xl border border-warning/20 bg-warning/5 p-5">
                ...
                <UInput
                    ...
                    @update:model-value="updateGoal($event)"
                />
            </div>
        </div>
        ...
        >
            <div
                class="absolute inset-0 bg-linear-to-b from-primary/5 to-transparent opacity-50"
            />
            <div class="relative flex h-36 items-end gap-2 lg:gap-3">
                <div
                    v-for="(point, index) in chartHeights"
                    :key="`${block.id}-chart-${index}`"
                    class="group relative flex-1 min-w-2 rounded-t-full bg-primary/20 transition-all hover:bg-primary/60"
                    :style="{ height: `${point}%` }"
                >
                    <div
            ...
            >
                <div class="flex items-center gap-2">
                    ...
                    <div
                        class="h-px flex-1 border-t border-dashed border-warning/70"
                    />
                </div>
            </div>
                ...
                >
                    <div class="min-w-0 flex-1">
                        <UInput
                            ...
                            class="w-full"
                            :ui="{
                                base: 'px-0 font-bold text-highlighted text-sm leading-tight',
                            }"
                            @update:model-value="
                                updateEntryLabel(entry.id, $event)
                            "
                        />
                        <p
                    ...
                    <div class="flex items-center gap-3">
                        <div class="w-24">
                            <UInput
                                ...
                                @update:model-value="
                                    updateEntryValue(entry.id, $event)
                                "
                            />
                        </div>
                        ...
                        <UButton
                            ...
                            @click="
                                removeTrackerEntry(tabId, block.id, entry.id)
                            "
                        />
                    </div>
                </div>
