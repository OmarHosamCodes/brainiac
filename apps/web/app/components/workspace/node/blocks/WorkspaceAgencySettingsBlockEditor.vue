...
watch(
  () => props.block.billingPeriodStartDay,
  (v) => {
    startDayRaw.value = String(v);
  },
);
watch(
  () => props.block.billingPeriodEndDay,
  (v) => {
...
function updateTeam(teamId: string | undefined) {
  mutateTypedBlock(props.tabId, props.block.id, "agency-settings", (entry) => {
    ...
    <div>
      <div class="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-xs text-zinc-500">Configure the recurring billing window.</p>
        </div>
      </div>
        ...
        <div class="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <UFormField label="Start day (1-28)" size="sm" class="mb-0">
              <div class="flex gap-2">
                <UInput v-model="startDayRaw" type="number" min="1" max="28" size="sm" />
                <UButton label="Apply" size="sm" variant="soft" @click="applyStartDay" />
              </div>
            </UFormField>
          </div>
          ...
          <div>
            <UFormField label="End day (1-28)" size="sm" class="mb-0">
              <div class="flex gap-2">
                <UInput v-model="endDayRaw" type="number" min="1" max="28" size="sm" />
                <UButton label="Apply" size="sm" variant="soft" @click="applyEndDay" />
              </div>
            </UFormField>
          </div>
        </div>
