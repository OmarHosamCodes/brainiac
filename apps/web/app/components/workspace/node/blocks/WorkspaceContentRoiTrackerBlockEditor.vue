...
function addItem() {
  mutateTypedBlock(props.tabId, props.block.id, "content-roi-tracker", (entry) => {
    entry.items.unshift(
      createWorkspaceContentRoiItem({
        title: "",
      }),
...
function updatePlatform(itemId: string, value: WorkspaceContentPlatform | string | undefined) {
  mutateItem(itemId, (entry) => {
    entry.platform =
      value === "instagram" || value === "tiktok" || value === "linkedin" || value === "youtube"
        ? value
        : "linkedin";
  });
}
...
function updateConversionInfluence(itemId: string, value: string | number | undefined) {
  mutateItem(itemId, (entry) => {
    entry.conversionInfluence = clampInteger(String(value ?? 0), 1, 10);
  });
}

function updateRepurposeValue(itemId: string, value: string | number | undefined) {
  mutateItem(itemId, (entry) => {
  ...
  <div class="space-y-6">
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60">Top Platform</p>
      ...
      <div class="rounded-3xl border border-muted/20 bg-elevated/10 p-5">
        <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60">Top Campaign</p>
        <p class="mt-2 text-2xl font-black tracking-tight text-highlighted sm:text-3xl">
      ...
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="space-y-2">
          <div class="flex flex-wrap items-center gap-2">
            ...
            <UBadge color="success" variant="soft" class="rounded-2xl px-3">
              {{ summary.highReturnCount }} high return
            </UBadge>
            <UBadge color="warning" variant="soft" class="rounded-2xl px-3">
              {{ promisingCount }} promising
            </UBadge>
            <UBadge color="error" variant="soft" class="rounded-2xl px-3">
        ...
        <template v-for="item in sortedItems" :key="item.id">
          ...
          <div class="flex items-center bg-default/40 p-3">
            <USelect
              ...
              @update:model-value="
                updatePlatform(item.id, $event as WorkspaceContentPlatform | undefined)
              "
            />
          </div>
          ...
          <div class="flex items-center bg-default/40 p-3">
            <UInput
              ...
              @update:model-value="updateCampaign(item.id, $event)"
            />
          </div>
          ...
          <div class="flex flex-col justify-center bg-default/40 p-3">
            ...
            <input
              ...
              @input="
                updateConversionInfluence(item.id, ($event.target as HTMLInputElement)?.value)
              "
            />
          </div>
          ...
          <div class="flex flex-col justify-center bg-default/40 p-3">
            ...
            <input
              ...
              @input="updateRepurposeValue(item.id, ($event.target as HTMLInputElement)?.value)"
            />
          </div>
              ...
              <div class="flex items-start justify-between gap-4">
                <div class="min-w-0 flex-1">
                  ...
                  <p class="mt-1 text-xs leading-relaxed opacity-70">
                    {{ getPerformanceSummary(getContentRoiScore(item)) }}
                  </p>
