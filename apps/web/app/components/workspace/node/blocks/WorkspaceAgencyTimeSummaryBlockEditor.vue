...
const dateRange = computed(() => {
    ...
    switch (datePreset.value) {
        ...
        case "custom": {
            from = customFromDate.value ? new Date(customFromDate.value) : new Date(now.getFullYear(), now.getMonth(), 1);
...
const summaryData = computed(
    () => summaryQuery.data.value?.summary ?? null,
);
...
function updateDatePreset(
    preset: "this-week" | "last-month" | "year-to-date" | "custom",
) {
...
<template>
    <div class="space-y-6">
        <!-- Filter bar -->
        <div class="space-y-4 rounded-2xl border border-zinc-200/30 bg-zinc-950/40 backdrop-blur-sm p-4 dark:border-zinc-800/50 dark:bg-zinc-950/50">
            <!-- Team & Date -->
            <div class="grid gap-3 md:grid-cols-4">
                ...
                <!-- Date preset -->
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-widest text-zinc-600 dark:text-zinc-400 mb-2">
                        Period
                    </label>
                    <div class="flex flex-wrap gap-2">
                        <UButton
                            ...
                            size="xs"
                            :variant="
                                datePreset === preset.value ? 'soft' : 'ghost'
                            "
                            :color="
                        ...
                        >
                            ...
                            <td class="px-4 py-3">
                                <div v-if="member.latestEntry">
                                    ...
                                    <p v-if="member.latestEntry.description" class="text-xs text-zinc-500 truncate mt-1">
        ...
        >
            ...
            />
            <p class="mt-4 font-medium text-zinc-400">
