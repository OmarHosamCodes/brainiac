                ...
                props.block.deals
                    ...
                    .sort(
                        (left, right) =>
                            ...
                            right.valueEgp - left.valueEgp,
                    ),
            ]),
        ) as Record<
            WorkspaceSalesForecastBucket,
            WorkspaceForecastConfidenceBoardBlock["deals"]
        >,
);
const bucketSummaryById = computed(
    () =>
        new Map(
            summary.value.bucketSummaries.map((entry) => [entry.bucket, entry]),
        ),
);
...
function mutateDeal(
    dealId: string,
    mutator: (
        deal: WorkspaceForecastConfidenceBoardBlock["deals"][number],
    ) => void,
) {
    mutateTypedBlock(
        props.tabId,
...
function moveDealByOffset(dealId: string, offset: -1 | 1) {
    const currentBucket = props.block.deals.find(
        (deal) => deal.id === dealId,
    )?.bucket;
    ...
    const currentIndex =
        WORKSPACE_SALES_FORECAST_BUCKETS.indexOf(currentBucket);
...
function canMoveDeal(bucket: WorkspaceSalesForecastBucket, offset: -1 | 1) {
    ...
    return (
        ...
        currentIndex + offset < WORKSPACE_SALES_FORECAST_BUCKETS.length
    );
}

function updateTargetRevenue(value: string | number | undefined) {
    mutateTypedBlock(
        props.tabId,
        props.block.id,
        "forecast-confidence-board",
        (entry) => {
            entry.targetRevenueEgp = toCurrencyValue(
                String(value ?? "0"),
                50000,
            );
        },
...
function updateExpectedCloseMonth(
    dealId: string,
    value: string | number | undefined,
) {
    mutateDeal(dealId, (deal) => {
        deal.expectedCloseMonth = String(value ?? "") || null;
    });
}

function updateOwner(dealId: string, value: string | number | undefined) {
    mutateDeal(dealId, (deal) => {
        deal.owner = String(value ?? "").slice(0, 120);
    });
}

function updateConfidence(dealId: string, value: number) {
    mutateDeal(dealId, (deal) => {
        deal.confidence = Math.max(10, Math.min(100, Math.round(value)));
    });
}
...
<template>
    <div class="space-y-8">
        ...
        <div class="grid gap-4 grid-cols-2 lg:grid-cols-4">
            <div
                class="group relative overflow-hidden rounded-[24px] bg-success/5 p-5 border border-success/10 transition-all hover:bg-success/10"
            >
                <div class="flex items-start justify-between">
                    <div>
                        <p
                            class="text-[10px] font-bold uppercase tracking-[0.2em] text-success/60"
                        >
                            Commit Revenue
                        </p>
                        <p
                            class="mt-2 text-3xl font-black tracking-tight text-success"
                        >
                            {{ formatCurrency(summary.commitRevenue) }}
                        </p>
                    </div>
                    <div
                        class="size-10 rounded-2xl bg-success/10 flex items-center justify-center"
                    >
                        <UIcon
                ...
                <p class="mt-4 text-xs text-success/60 leading-relaxed">
                    Guaranteed closing value
                </p>
            </div>

            <div
                class="group relative overflow-hidden rounded-[24px] bg-primary/5 p-5 border border-primary/10 transition-all hover:bg-primary/10"
            >
                <div class="flex items-start justify-between">
                    <div>
                        <p
                        ...
                        <p
                            class="mt-2 text-3xl font-black tracking-tight text-primary"
                        >
                ...
                <p class="mt-4 text-xs text-primary/60 leading-relaxed">
                    Adjusted for confidence
                </p>
            </div>

            <div
                class="group relative overflow-hidden rounded-[24px] bg-error/5 p-5 border border-error/10 transition-all hover:bg-error/10"
            >
                <div class="flex items-start justify-between">
                    <div>
                        <p
                            class="text-[10px] font-bold uppercase tracking-[0.2em] text-error/60"
                        >
                            At-Risk Value
                        </p>
                        <p
                            class="mt-2 text-3xl font-black tracking-tight text-error"
                        >
                            {{ formatCurrency(summary.atRiskValue) }}
                        </p>
                    </div>
                    <div
                        class="size-10 rounded-2xl bg-error/10 flex items-center justify-center"
                    >
                        <UIcon
                ...
                <p class="mt-4 text-xs text-error/60 leading-relaxed">
                    Low confidence deals
                </p>
            </div>

            <div
                class="group relative overflow-hidden rounded-[24px] bg-warning/5 p-5 border border-warning/10 transition-all hover:bg-warning/10"
            >
                <div class="flex items-start justify-between">
                    <div>
                        <p
                            class="text-[10px] font-bold uppercase tracking-[0.2em] text-warning/60"
                        >
                            Coverage vs Target
                        </p>
                        <p
                    ...
                    <div
                        class="size-10 rounded-2xl bg-warning/10 flex items-center justify-center"
                    >
                        <UIcon
                ...
                <p class="mt-4 text-xs text-warning/60 leading-relaxed">
        ...
        <div class="flex flex-wrap items-center justify-between gap-6 px-1">
            <div class="max-w-md">
                ...
                <p class="text-xs text-muted mt-1 leading-relaxed">
                    Manage your sales pipeline by deal confidence and track
            ...
            <div class="flex flex-wrap items-center gap-6">
                <div
                    class="flex items-center gap-3 bg-elevated/5 p-1.5 rounded-2xl border border-muted/10"
                >
                    <label
                        class="ml-3 text-[10px] font-black uppercase tracking-[0.2em] text-muted/40 whitespace-nowrap"
                        >Target</label
                    >
                    <UInput
                        ...
                        @update:model-value="updateTargetRevenue($event)"
                    />
                </div>
        ...
        <!-- Board Grid -->
        <div
            class="overflow-x-auto pb-6 -mx-1 px-1 scrollbar-thin scrollbar-thumb-muted/20 snap-x snap-mandatory"
        >
            <div class="flex gap-6">
                <section
                    ...
                    :class="[
                        ...
                        dragOverBucket === bucket
                            ? 'ring-2 ring-primary/30 brightness-110 shadow-xl'
                            : '',
                    ]"
                ...
                >
                    ...
                    <div class="p-4">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div
                                    class="size-8 rounded-xl bg-default/80 flex items-center justify-center border border-muted/10 shadow-sm"
                                >
                                    <UIcon
                                        :name="getBucketClasses(bucket).icon"
                                        class="size-4"
                                        :class="getBucketClasses(bucket).text"
                                    />
                                </div>
                                <div>
                                    <p
                                        class="text-[10px] font-black uppercase tracking-[0.2em] text-muted/50 leading-none"
                                    >
                                        {{
                                            workspaceSalesForecastBucketLabels[
                                                bucket
                                    ...
                                    <p
                                        class="mt-1 text-xs font-bold text-highlighted/60 leading-none"
                                    >
                                        {{ getBucketSummary(bucket).dealCount }}
                                        deals
                                    </p>
                                </div>
                            </div>
                        ...
                        <div class="mt-6 flex items-baseline justify-between">
                            <p
                                class="text-2xl font-black tracking-tight text-highlighted"
                            >
                                {{
                                    formatCurrency(
                                        getBucketSummary(bucket).totalValue,
                                    )
                                }}
                            </p>
                            <p
                                class="text-[10px] font-bold uppercase tracking-wider text-muted/40"
                            >
                                Wgt:
                                {{
                                    formatCurrency(
                                        getBucketSummary(bucket).weightedValue,
                                    )
                                }}
                            </p>
                        </div>
                    ...
                    <div class="flex-1 space-y-3 p-2 min-h-[400px]">
                            ...
                            <div class="flex items-start justify-between gap-3">
                                <div class="min-w-0 flex-1">
                                    <UInput
                                        ...
                                        @update:model-value="
                                            updateClientName(deal.id, $event)
                                        "
                                    />
                                    <div
                                        class="mt-2 flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-wider text-muted/40"
                                    >
                                        <UBadge
                                            color="neutral"
                                            variant="soft"
                            ...
                            <div class="mt-6 space-y-4">
                                <div class="grid grid-cols-2 gap-3">
                                    <div class="space-y-1.5">
                                        <label
                                            class="text-[9px] font-bold uppercase tracking-widest text-muted/40 ml-1"
                                            >VALUE (EGP)</label
                                        >
                                        <UInput
                                            ...
                                            size="xs"
                                            :ui="{
                                                base: 'rounded-xl font-bold bg-elevated/5 border-muted/10',
                                            }"
                                            @update:model-value="
                                                updateValue(deal.id, $event)
                                            "
                                        />
                                    </div>
                                    <div class="space-y-1.5">
                                        ...
                                        <UInput
                                            ...
                                            size="xs"
                                            :ui="{
                                                base: 'rounded-xl bg-elevated/5 border-muted/10',
                                            }"
                                            @update:model-value="
                                                updateExpectedCloseMonth(
                                                    deal.id,
                                                    $event,
                                                )
                                            "
                                ...
                                <div class="pt-2">
                                    ...
                                    >
                                        <span
                                            class="text-[9px] font-bold uppercase tracking-widest text-muted/40"
                                            >Confidence</span
                                        >
                                        <span
                                            class="text-[10px] font-black text-highlighted bg-primary/10 px-1.5 py-0.5 rounded-md"
                                            >{{ deal.confidence }}%</span
                                        >
                                    </div>
                                    <USlider
                                ...
                                <div
                                    class="space-y-1.5 bg-elevated/5 p-3 rounded-2xl border border-muted/5"
                                >
                                    <label
                                        class="text-[9px] font-bold uppercase tracking-widest text-muted/40 flex items-center gap-1.5"
                                    >
                                        <UIcon
                                    ...
                                    <UTextarea
                                        ...
                                        :rows="1"
                                        :ui="{
                                            base: 'p-0 text-[11px] leading-relaxed text-highlighted/80 placeholder:text-muted/20',
                                        }"
                                        @update:model-value="
                                            updateNextAction(deal.id, $event)
                                        "
                                    />
                                </div>
                            </div>
                        ...
                        >
                            <div
                                class="size-12 rounded-full bg-muted/5 flex items-center justify-center mb-3"
                            >
                                <UIcon
                                    :name="getBucketClasses(bucket).icon"
                                    class="size-6 text-muted/20"
                                />
                            </div>
                            <p
                                class="text-[10px] font-bold uppercase tracking-widest text-muted/30"
                            >
                                Empty {{ bucket }}
                            </p>
                        </div>
