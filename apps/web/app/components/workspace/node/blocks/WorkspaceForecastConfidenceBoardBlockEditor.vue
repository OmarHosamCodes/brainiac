...
function mutateDeal(
  ...
  mutateTypedBlock(props.tabId, props.block.id, "forecast-confidence-board", (entry) => {
...
function moveDealByOffset(dealId: string, offset: -1 | 1) {
  const currentBucket = props.block.deals.find((deal) => deal.id === dealId)?.bucket;
...
function updateTargetRevenue(value: string | number | undefined) {
  mutateTypedBlock(props.tabId, props.block.id, "forecast-confidence-board", (entry) => {
    entry.targetRevenueEgp = toCurrencyValue(String(value ?? "0"), 50000);
  });
}
...
function updateExpectedCloseMonth(dealId: string, value: string | number | undefined) {
  mutateDeal(dealId, (deal) => {
              ...
              <div class="mt-6 space-y-4">
                <div class="grid grid-cols-2 gap-3">
                  <div class="space-y-1.5">
                    <label class="text-[9px] font-bold uppercase tracking-widest text-muted/40 ml-1"
                      >VALUE (EGP)</label
                    >
                    <UInput
                  ...
                  <div class="space-y-1.5">
                    ...
                    <UInput
                      ...
                      @update:model-value="updateExpectedCloseMonth(deal.id, $event)"
                    />
                  </div>
                </div>
                ...
                <div class="space-y-1.5">
                  ...
                  <UInput
                    ...
                    @update:model-value="updateOwner(deal.id, $event)"
                  />
                </div>
                ...
                <div class="pt-2">
                  <div class="flex items-center justify-between mb-2 px-1">
                    <span class="text-[9px] font-bold uppercase tracking-widest text-muted/40"
                      >Confidence</span
                    >
                    <span
                  ...
                  <USlider
                    ...
                    @update:model-value="
                      updateConfidence(
                        deal.id,
                        typeof $event === 'number' ? $event : deal.confidence,
                      )
                    "
                  />
                </div>
