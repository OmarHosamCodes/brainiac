...
function sourceHasTaskBlocks(sourceNode: WorkspaceNode) {
  return sourceNode.tabs.some((tab) =>
    tab.blocks.some((block) => block.type === "task-list" || block.type === "eisenhower-matrix"),
  );
}

function getSourceTaskCount(sourceNodeId: string) {
  return collectedTasks.value.filter((item) => item.sourceNodeId === sourceNodeId).length;
}
  ...
  >
    <template #body>
      ...
      <section>
        ...
        >
          <UIcon name="i-lucide-plug-2" class="mx-auto size-8 text-muted/30" />
        ...
        <ul v-else class="mt-3 space-y-2">
          ...
          >
            <div class="flex min-w-0 items-center gap-3">
              <UIcon name="i-lucide-box" class="size-4 shrink-0 text-muted/60" />
              <button
                type="button"
      ...
      <section>
        <div class="flex flex-wrap items-center justify-between gap-3">
          <p class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60">Task Browser</p>
          <div class="flex items-center gap-1">
            <UButton
              ...
              :key="mode.key"
              size="xs"
              :color="groupMode === mode.key ? 'primary' : 'neutral'"
              :variant="groupMode === mode.key ? 'soft' : 'ghost'"
              :icon="mode.icon"
              class="rounded-lg"
        ...
        <div
          ...
          class="mt-3 rounded-2xl border border-dashed border-muted/20 bg-elevated/5 py-8 text-center"
        >
          <UIcon name="i-lucide-inbox" class="mx-auto size-8 text-muted/30" />
          <p class="mt-2 text-sm text-muted">
          ...
          >
            ...
            <div class="flex items-center justify-between gap-3 px-4 py-3">
              ...
              <UButton
                v-if="
                  group.sourceNodeId &&
                  sourceHasTaskBlocks(allNodes.find((n) => n.id === group.sourceNodeId)!)
                "
                size="xs"
                color="primary"
                variant="ghost"
            ...
            <ul class="border-t border-muted/10">
              ...
              >
                ...
                >
                  <UIcon v-if="item.task.completed" name="i-lucide-check" class="size-3" />
                </button>

                <!-- Task Text -->
                <p
                  class="min-w-0 flex-1 truncate text-sm"
                  :class="item.task.completed ? 'text-muted line-through' : 'text-highlighted'"
                >
                  {{ item.task.text || "Untitled task" }}
                </p>

                <!-- Priority Dropdown -->
                <UDropdownMenu v-if="canEditPriority(item)" :items="priorityMenuItems(item)">
                  <UButton
                    size="xs"
                    variant="ghost"
                    class="rounded-lg"
                ...
                <button
                  ...
                  @click="handleNavigate(item.sourceNodeId)"
                >
    ...
    <template #footer>
      <UButton color="neutral" variant="ghost" class="rounded-xl" @click="closeModal">
        Close
      </UButton>
    </template>
  </UModal>
