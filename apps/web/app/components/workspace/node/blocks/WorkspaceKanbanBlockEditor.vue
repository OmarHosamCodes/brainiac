...
const boardSummary = computed(() => ({
  columnCount: props.block.columns.length,
  cardCount: props.block.cards.length,
  assignedCount: props.block.cards.filter((card) => Boolean(card.assignee?.trim())).length,
  scheduledCount: props.block.cards.filter((card) => Boolean(card.dueDate)).length,
}));
...
function updateColumnTitle(columnId: string, value: string | number | undefined) {
  mutateKanbanColumn(props.tabId, props.block.id, columnId, (entry) => {
    entry.title = String(value ?? "").slice(0, 80);
  });
}

function updateCardTitle(cardId: string, value: string | number | undefined) {
  mutateKanbanCard(props.tabId, props.block.id, cardId, (entry) => {
    entry.title = String(value ?? "").slice(0, 240);
  });
}

function updateCardDescription(cardId: string, value: string | number | undefined) {
  mutateKanbanCard(props.tabId, props.block.id, cardId, (entry) => {
    entry.description = String(value ?? "").slice(0, 4000);
  });
}

function updateCardAssignee(cardId: string, value: string | number | undefined) {
  mutateKanbanCard(props.tabId, props.block.id, cardId, (entry) => {
    entry.assignee = String(value ?? "").slice(0, 120);
  });
}

function updateCardDueDate(cardId: string, value: string | number | undefined) {
  mutateKanbanCard(props.tabId, props.block.id, cardId, (entry) => {
    ...
    <div class="space-y-4 rounded-3xl border border-muted/20 bg-elevated/10 p-5">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="space-y-1">
          ...
          <p class="mt-1 text-xs text-muted">Drag cards to advance workflow</p>
        </div>
        <UButton
          color="primary"
          variant="subtle"
      ...
      >
        ...
        <div class="mb-4 flex items-center justify-between px-2">
          <div class="flex items-center gap-2 min-w-0 flex-1">
            ...
            <UInput
              ...
              @update:model-value="updateColumnTitle(column.id, $event)"
            />
            <span
              class="text-[10px] font-bold text-muted/60 bg-elevated/10 px-1.5 py-0.5 rounded-md"
              >{{ cardsByColumn[column.id]?.length || 0 }}</span
            >
          </div>
          ...
          <UButton
            ...
            class="rounded-lg text-muted/70 transition-colors hover:text-error/80"
        ...
        <div class="flex-1 space-y-3">
          ...
          <div
            ...
            class="rounded-2xl border border-dashed border-muted/20 bg-elevated/5 py-8 text-center text-xs font-medium text-muted/40"
          >
          ...
          >
            <div class="flex items-start gap-3">
              <div class="flex-1 min-w-0">
                <UInput
                  ...
                  class="w-full"
                  :ui="{
                    base: 'px-0 py-0 font-bold text-highlighted text-sm leading-tight placeholder:text-muted/30',
                  }"
                  @update:model-value="updateCardTitle(card.id, $event)"
                />
                <p
              ...
              <UButton
                color="neutral"
                variant="ghost"
                :icon="expandedCardId === card.id ? 'i-lucide-chevron-up' : 'i-lucide-expand'"
                size="xs"
            ...
            >
              <div class="space-y-1">
                <label class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60 px-1"
                  >Description</label
                >
                <UTextarea
                  ...
                  :ui="{
                    base: 'text-sm text-toned leading-relaxed bg-elevated/5',
                  }"
                  @update:model-value="updateCardDescription(card.id, $event)"
                />
              </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="space-y-1">
                  ...
                  <UInput
                    ...
                    icon="i-lucide-user"
                    class="rounded-xl"
                    @update:model-value="updateCardAssignee(card.id, $event)"
                  />
                </div>
                ...
                <div class="space-y-1">
                  ...
                  <UInput
                    ...
                    class="rounded-xl"
                    @update:model-value="updateCardDueDate(card.id, $event)"
                  />
                </div>
              </div>
            ...
            <!-- Card Footer Meta -->
            <div
              v-else-if="card.assignee || card.dueDate"
