...
const boardSummary = computed(() => ({
    ...
    assignedCount: props.block.cards.filter((card) =>
        Boolean(card.assignee?.trim()),
    ).length,
    scheduledCount: props.block.cards.filter((card) => Boolean(card.dueDate))
        .length,
}));
    ...
    <div class="space-y-6">
        ...
        >
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div class="space-y-1">
                    <h3
                        class="text-sm font-bold text-highlighted uppercase tracking-wider"
                    >
                        Flow Board
                    </h3>
                    <p class="mt-1 text-xs text-muted">
                        Drag cards to advance workflow
                    </p>
                </div>
                <UButton
                    color="primary"
            ...
            >
                ...
                <div class="mb-4 flex items-center justify-between px-2">
                    <div class="flex items-center gap-2 min-w-0 flex-1">
                        <span
                            class="size-2 rounded-full bg-primary/60 shrink-0"
                        />
                        <UInput
                            ...
                            @update:model-value="
                                updateColumnTitle(column.id, $event)
                            "
                        />
                        <span
                ...
                <div class="flex-1 space-y-3">
                    ...
                    <article
                        ...
                        :class="[
                            ...
                            expandedCardId === card.id
                                ? 'ring-2 ring-primary/20 bg-elevated/5 shadow-inner'
                                : '',
                        ]"
                        draggable="true"
                    ...
                    >
                        ...
                        >
                            <div class="space-y-1">
                                <label
                                    class="text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60 px-1"
                                    >Description</label
                                >
                                <UTextarea
                                    ...
                                    @update:model-value="
                                        updateCardDescription(card.id, $event)
                                    "
                                />
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div class="space-y-1">
                                    ...
                                    <UInput
                                        ...
                                        class="rounded-xl"
                                        @update:model-value="
                                            updateCardAssignee(card.id, $event)
                                        "
                                    />
                                </div>
                                ...
                                <div class="space-y-1">
                                    ...
                                    <UInput
                                        ...
                                        class="rounded-xl"
                                        @update:model-value="
                                            updateCardDueDate(card.id, $event)
                                        "
                                    />
                                </div>
                            </div>
