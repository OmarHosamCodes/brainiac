...
function updateMetricLabel(metricId: string, value: string | number | undefined) {
  mutateScorecardMetric(props.tabId, props.block.id, metricId, (entry) => {
    entry.label = String(value ?? "").slice(0, 120);
  });
}

function updateMetricValue(metricId: string, value: string | number | undefined) {
  mutateScorecardMetric(props.tabId, props.block.id, metricId, (entry) => {
    entry.value = toNumber(value, 0);
  });
}

function updateMetricTarget(metricId: string, value: string | number | undefined) {
  mutateScorecardMetric(props.tabId, props.block.id, metricId, (entry) => {
    entry.target = toNumber(value, 100);
  });
}

function updateMetricUnit(metricId: string, value: string | number | undefined) {
  mutateScorecardMetric(props.tabId, props.block.id, metricId, (entry) => {
  ...
  <div class="space-y-8">
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      ...
      >
        ...
        <p class="mt-1 text-2xl font-black tracking-tight text-primary sm:text-3xl">
          {{ summary.metricCount }}
        </p>
      </div>
      ...
      >
        ...
        <p class="mt-1 text-2xl font-black tracking-tight text-success sm:text-3xl">
          {{ summary.atTarget }}
        </p>
      </div>
      ...
      >
        ...
        <p class="mt-1 text-2xl font-black tracking-tight text-error sm:text-3xl">
      ...
      >
        ...
        <p class="mt-1 text-2xl font-black tracking-tight text-warning sm:text-3xl">
          {{ summary.avgProgress }}%
        </p>
      </div>
    </div>
    ...
    <div class="space-y-4">
      ...
      <div v-if="block.metrics.length > 0" class="grid gap-4 md:grid-cols-2">
        <article
          v-for="metric in block.metrics"
          :key="metric.id"
          class="rounded-3xl border p-5 transition-colors"
          :class="getMetricStatus(metric.value, metric.target).cardClass"
        >
          <div class="flex items-start justify-between gap-4">
            <div class="min-w-0 flex-1 space-y-2">
              <UInput
                ...
                @update:model-value="updateMetricLabel(metric.id, $event)"
              />

              <div class="flex flex-wrap items-center gap-2">
                <UBadge
                  :color="getMetricStatus(metric.value, metric.target).badgeColor"
                  variant="soft"
                  class="rounded-2xl"
                >
                  {{ getMetricStatus(metric.value, metric.target).label }}
                </UBadge>
                <UBadge color="neutral" variant="soft" class="rounded-2xl">
                  {{ getMetricProgress(metric.value, metric.target) }}% progress
                </UBadge>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="mt-2 flex items-end gap-2">
                <span
                  ...
                  :class="getMetricStatus(metric.value, metric.target).textClass"
                >
                  {{ metric.value }}
                </span>
                <span class="pb-1 text-[10px] font-bold uppercase tracking-[0.2em] text-muted/60">
                  / {{ metric.target }}
                  {{ metric.unit || "units" }}
                </span>
              </div>
          ...
          <div class="mt-5 space-y-3">
            ...
            >
              ...
              <span :class="getMetricStatus(metric.value, metric.target).textClass">
          ...
          <div class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div class="space-y-1">
              ...
              <UInput
                ...
                @update:model-value="updateMetricValue(metric.id, $event)"
              />
            </div>
            ...
            <div class="space-y-1">
              ...
              <UInput
                ...
                @update:model-value="updateMetricTarget(metric.id, $event)"
              />
            </div>
            ...
            <div class="space-y-1">
              ...
              <UInput
                ...
                @update:model-value="updateMetricUnit(metric.id, $event)"
              />
            </div>
          </div>
        </article>
      ...
      >
        ...
        <p class="mt-2 text-sm text-muted">Add a metric to track performance against a target.</p>
        <UButton
          color="primary"
          variant="soft"
          size="sm"
