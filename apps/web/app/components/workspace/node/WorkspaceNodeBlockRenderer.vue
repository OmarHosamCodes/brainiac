...
const registryEntry = computed(() =>
    getWorkspaceBlockRegistryEntry(props.block.type),
);
const editorComponent = computed<Component>(
    () => registryEntry.value.component as Component,
);
const isContextBlock = computed(() =>
    isAgentContextBlock(props.tabId, props.block.id),
);
...
<template>
    ...
    >
            ...
            <div class="flex min-w-0 flex-1 items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                    <UInput
                        ...
                        @update:model-value="
                            updateBlockTitle(tabId, block.id, $event ?? '')
                        "
                    />
                    <p
                        class="text-[11px] font-semibold uppercase tracking-widest text-muted/80"
                    >
                        {{ registryEntry.label }}
                    </p>
                </div>
        ...
        <div
            v-if="
                normalizedBlockSearch && getBlockSearchMatches(block).length > 0
            "
