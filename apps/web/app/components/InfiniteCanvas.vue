...
function selectNode(nodeId: string, isToggle = false) {
    ...
    if (
        ...
        props.selectedNodeIds[0] !== nodeId
    ) {
        setSelection([nodeId]);
    }
}
...
function onNodeShellPointerDown(event: PointerEvent, node: CanvasNodeModel) {
    ...
    const selectedIds = selectedNodeIdSet.value.has(node.id)
    ...
    updateNodes(
        props.nodes.map((node) =>
            node.id === interaction.nodeId
                ? resizeNodeRect(startNode, deltaX, deltaY, handle)
                : node,
        ),
    );
}
...
function onViewportPointerDown(event: PointerEvent) {
    ...
    const shouldPan =
        event.button === 1 || (event.button === 0 && isSpacePressed.value);
...
const minimapViewfinder = computed(() =>
    projectToMinimap(visibleWorldRect.value, 24, 24),
);
...
function navigateFromMinimap(event: PointerEvent) {
    ...
    const worldX =
        minimapScene.value.x + (localX - MINIMAP_PADDING) / minimapScale.value;
    const worldY =
        minimapScene.value.y + (localY - MINIMAP_PADDING) / minimapScale.value;
...
watch(
    ...
    (nodeIds) => {
        const nextSelection = props.selectedNodeIds.filter((nodeId) =>
...
<template>
    ...
    >
                ...
                <div class="canvas-plane" :style="canvasStyle">
                    ...
                    >
                        ...
                        >
                            ...
                            <div
                                class="canvas-node-content min-h-0 flex-1 p-1.5"
                            >
                                <slot
                                    name="node"
                                    :node="node"
                            ...
                            <button
                                ...
                                @pointerdown.stop="
                                    beginResize($event, node, handle.edge)
                                "
                            />
                        </div>
                    </article>
        ...
        >
            ...
            >
                ...
                <p class="mt-3 text-sm leading-6 text-toned">
                    ...
                    Every card is user-defined, so each workspace can evolve
                    independently.
                </p>
            </div>
        </div>
        ...
        >
            <div
                class="flex items-center justify-between gap-3 border-b border-muted/60 px-4 py-3"
            >
                <div>
                    <p
                        class="text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-muted"
                    ...
                    <p class="mt-1 text-xs text-toned">
                        ...
                        {{ selectedNodes.length || 0 }} selected
                    </p>
                </div>
                <div
