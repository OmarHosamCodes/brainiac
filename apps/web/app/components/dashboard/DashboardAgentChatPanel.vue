...
<template>
    <section
        class="flex h-full flex-col overflow-hidden rounded-[2rem] border border-zinc-200/50 dark:border-zinc-800/50 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-2xl shadow-2xl ring-1 ring-black/[0.03] dark:ring-white/[0.02]"
    >
        <div class="flex items-center justify-between px-6 py-5 border-b border-zinc-200/30 dark:border-zinc-800/30">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-sm font-bold tracking-tight text-zinc-900 dark:text-zinc-100">Copilot</h2>
                    <p class="text-[10px] font-medium text-zinc-500 uppercase tracking-widest">{{ scopeLabel }}</p>
                </div>
            </div>
        ...
        >
            <template v-if="messages.length === 0">
                <div class="space-y-6">
                    <div class="p-5 rounded-3xl bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-200/50 dark:border-zinc-800/50">
                        <p class="text-xs font-semibold text-zinc-900 dark:text-zinc-100 mb-1">How can I help?</p>
                        <p class="text-xs text-zinc-500 leading-relaxed">I can analyze your nodes, find connections, or help you brainstorm new ideas.</p>
                    </div>

                    <div class="space-y-2">
                        <p class="text-[10px] font-bold text-zinc-400 uppercase tracking-widest ml-1">Suggestions</p>
            ...
            >
                <div
                    ...
                    :class="[
                        message.role === 'user'
                            ? 'rounded-[1.5rem] rounded-tr-none bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 shadow-xl shadow-black/5'
                            : 'rounded-[1.5rem] rounded-tl-none bg-white dark:bg-zinc-900 border border-zinc-200/50 dark:border-zinc-800/50 text-zinc-800 dark:text-zinc-200 shadow-xl shadow-black/5'
                    ]"
                >
                ...
                >
                    <span 
                        ...
                        :key="title"
                        class="text-[9px] font-bold uppercase tracking-widest text-zinc-400"
                    >
                        · {{ title }}
                    </span>
                </div>
        ...
        <div class="p-6 border-t border-zinc-200/30 dark:border-zinc-800/30 bg-zinc-50/50 dark:bg-zinc-900/20">
            <div class="relative">
                <UTextarea
                    v-model="draft"
                    :rows="2"
            ...
            <div class="mt-4 flex items-center justify-between gap-4">
                <div class="flex items-center gap-2 overflow-hidden">
                    <USelectMenu
                        ...
                        color="neutral"
                        :ui="{ 
                            base: 'text-[10px] font-bold uppercase tracking-widest text-zinc-400 bg-transparent border-none p-0 h-auto'
                        }"
                    />
                </div>
                ...
                <div v-if="selectedNodes.length > 0" class="flex -space-x-2">
                    ...
                    <div v-if="selectedNodes.length > 3" class="size-5 rounded-full border border-white dark:border-zinc-950 bg-zinc-200 dark:bg-zinc-800 flex items-center justify-center text-[8px] text-zinc-500 font-bold">
